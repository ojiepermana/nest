...
export class BusinessEntityRepository {
   ...
   */
  async create(dto: CreateBusinessEntityDto): Promise<any> {
    const columns = Object.keys(dto);
    const values = Object.values(dto);
    const placeholders = values.map((_, i) => `$${i + 1}`).join(', ');
    
    const query = `
      INSERT INTO "entity"."business_entity" (${columns.join(', ')})
      VALUES (${placeholders})
      RETURNING *
    `;
  ...
  async findAll(): Promise<any[]> {
    const query = `SELECT * FROM "entity"."business_entity" ORDER BY id`;
  ...
  async findOne(id: string): Promise<any | null> {
    const query = `SELECT * FROM "entity"."business_entity" WHERE id = $1`;
  ...
  async findWithFilters(
    filter: BusinessEntityFilterDto,
    ...
    // Get total count
    const countQuery = `SELECT COUNT(*) as total FROM "entity"."business_entity" ${whereClause}`;
    ...
    // Get paginated data
    const dataQuery = `SELECT * FROM "entity"."business_entity" ${whereClause} ${orderByClause} LIMIT $${paramIndex} OFFSET $${paramIndex + 1}`;
   ...
   */
  async update(id: string, dto: UpdateBusinessEntityDto): Promise<any | null> {
  ...
  async delete(id: string): Promise<boolean> {
    const query = `DELETE FROM "entity"."business_entity" WHERE id = $1`;
    const result = await this.pool.query(query, [id]);
  ...
  async count(): Promise<number> {
    const query = `SELECT COUNT(*) as count FROM "entity"."business_entity"`;
    const result = await this.pool.query(query);
  ...
  async exists(id: string): Promise<boolean> {
    const query = `SELECT 1 FROM "entity"."business_entity" WHERE id = $1 LIMIT 1`;
    const result = await this.pool.query(query, [id]);
